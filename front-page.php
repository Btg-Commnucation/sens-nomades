...
if ($query->have_posts()) :
    while ($query->have_posts()) :
        ...
        if ($categories[0]->name !== 'Options') :
            ...
            $resume_product->recommande = get_field('produit_recommande');
            ...
            $resume_product->birthday  = get_the_date('d/m/Y');
...
<main id="front">
    ...
    <section class="recommanded-travel">
        <div class="container-narrow">
            ...
            <div class="voyages-link">
                ...
                <a href="<?= esc_url($voyages_link['url']); ?>" class="btn voyages-btn"><?= esc_html($voyages_link['title']); ?></a>
            </div>
        </div>
    </section>
    <section class="map">
        ...
        <div class="container-narrow">
            <div class="privatiser__content">
                ...
                <a href="<?= esc_url($lien_privatiser['url']); ?>" target="<?= esc_attr($lien_carte_target); ?>"><?=
                                                                                                                    esc_html($lien_privatiser['title']);
                                                                                                                    ?></a>
            </div>
            ...
            ?>
                <div class="directions__container">
                    ...
                    <div class="direction" v-for="(direction, index) in directions" :key="index">
                        ...
                        <div class="open">
                            <img src="<?= get_template_directory_uri() ?>/assets/open.svg" alt="Ouvrir le détail" class="open" @click="toggleElement(index)" v-if="!directionsShow[index].show">
                            <img src="<?= get_template_directory_uri() ?>/assets/close.svg" alt="Fermer le détail" class="close" @click="toggleElement(index)" v-if="directionsShow[index].show">
                        </div>
    ...
    <section class="actualites">
        <div class="container-narrow">
            ...
            ?>
            <a href="<?= esc_url($lien_actu['url']); ?>" class="btn btn-actu" target="<?= esc_attr($lien_actu_target); ?>">Tous nos articles</a>
        </div>
    </section>

    <? ph // get_template_part( 'parts/instagram' ); 
    ?>
    ...
    <script>
        ...
        createApp({
            ...
            methods: {
                ...
                getPost() {
                    let tempData = <?= json_encode($resume_product_list) ?>;
                    tempData.forEach(data => {
                        if (data.recommande === 'Oui') {
                            this.posts.push(data);
                        }
                    })
                    this.postsMap = <?= json_encode($resume_product_list) ?>;
                    this.posts.sort((a, b) => {
                        let newB = b.birthday.split('/');
                        let newA = a.birthday.split('/');
